...
def layout_text(words, font):
    ...
    for line in text_lines:
        total_word_width = sum(draw.textbbox((0,0), w, font=font)[2]-draw.textbbox((0,0), w, font=font)[0] for w in line)
    ...
    return word_positions, text_lines

def draw_text_frame(word_positions, text_lines, current_time, font):
    ...
    page = max_line // MAX_LINES_ON_SCREEN
    start_line = page * MAX_LINES_ON_SCREEN
    end_line = start_line + MAX_LINES_ON_SCREEN
    visible_lines = text_lines[start_line:end_line]

    total_height = (FONT_SIZE + LINE_SPACING) * len(visible_lines) - LINE_SPACING
    ...
    for i in visible_word_indices:
        ...
        if start_line <= line_idx < end_line:
            ...
            y = y_start + (line_idx - start_line) * (FONT_SIZE + LINE_SPACING)
...
def generate_lyrics_video(mp3_path, lrc_path, out_path, fps=DEFAULT_FPS, font_gui=FONT_NAME):
    ...
    word_positions, text_lines = layout_text(words, font)
    num_frames = int(duration * fps)

    for i in range(num_frames):
        current_time = i / fps
        frame_np = draw_text_frame(word_positions, text_lines, current_time, font)
